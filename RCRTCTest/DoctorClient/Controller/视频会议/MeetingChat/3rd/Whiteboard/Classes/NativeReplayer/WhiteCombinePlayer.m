...
{
    if (self = [self initWithNativePlayer:nativePlayer]) {
...
{
    DLOG(@"first updateWhitePlayerPhase %@ pauseReason:%@", @(phase), @(self.pauseReason));
    ...
    DLOG(@"end updateWhitePlayerPhase %@ pauseReason:%@", @(phase), @(self.pauseReason));
}
